<template>
  <q-dialog v-model="isDialogShowing" no-backdrop-dismiss no-esc-dismiss>
    ...
    <q-card class="column no-wrap full-width" style="max-height:88%" v-if="isPageReady">
      ...
      <q-card-section>
        <q-form ref="editModelGroupFormRef" @submit="updateModelGroupHandler">
      ...
      <q-card-section class="row justify-around items-center">
        <q-btn label="Cancel" color="primary" rounded outline no-caps unelevated style="width: 120px;"
          @click="$routerBack({ name: 'ModelGroupList' })" dense :disable="isModelGroupUpdating" />
        <q-btn label="Update" color="primary" rounded no-caps unelevated style="width: 120px;" dense
          @click="$refs.editModelGroupFormRef.submit()" :loading="isModelGroupUpdating" :disable="!isFormEdited" />
      </q-card-section>
...
<script setup>
import { ref } from 'vue';
import { useModelGroupEdit } from '../../composables';
